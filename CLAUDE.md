...
`e2e/mobile/whoami.spec.ts` has four extra mobile-only tests guarding `WhoAmIGame.tsx`'s small-screen behavior — a real on-screen keyboard turned out to need several layered fixes, each with its own regression test:

- **Keyboard-safe layout (height)**: the component tracks `window.visualViewport.height` in state and sizes its root container from that (instead of `100dvh`), because on both iOS Safari and Chrome an on-screen keyboard shrinks `visualViewport` but leaves the CSS layout viewport (and `dvh`) untouched — `dvh` alone never reacts to the keyboard. `page.setViewportSize` doesn't reproduce this either, since it resizes the real layout viewport. So the test instead: uses `page.clock` to fast-forward through several of `whoami-jordan`'s clue auto-reveals (stacking enough clue cards to matter, no real wall-clock wait), then directly overrides `window.visualViewport.height` and dispatches the `resize` event the component listens for — the same signal a real keyboard opening sends — and asserts the guess input's bounding rect stays within that shrunk height and the first clue stays in the (real, unresized) viewport. Playwright cannot open a real OS keyboard, so this is a deliberate approximation of its effect, not a substitute for manual testing on a real device.
- **Auto-scroll to the latest clue**: the clue list scrolls itself (`scrollIntoView` on a sentinel div) whenever `visibleClues` changes, so a newly auto-revealed clue is never hidden below the fold on a small screen — the scroll itself uses `behavior: "auto"` (instant), not `"smooth"`, because an animated scroll's duration depends on the browser actually getting around to running it, which isn't guaranteed under CPU contention. This test went through three iterations before it stopped being flaky under the full parallel suite (12 tests, up to 10 workers) — each iteration removed one more dependency on something *real* (wall-clock time, a real animation, or a real browser-dispatched event) in favor of something the test controls directly, in order:
  1. Waiting on `whoami-livingston`'s real 5s `clueInterval` timers was flaky (~1-in-3) purely from other concurrent browsers stealing CPU and delaying this page's `setInterval` past any reasonable assertion timeout.
  2. Switching clue reveals to `page.clock` (stepped one `runFor(5_000)` interval at a time, syncing on each clue's visibility before the next step — jumping multiple intervals in one `runFor()` call is separately flaky, some ticks don't reliably commit their state update) fixed that, but a *real* `page.setViewportSize()` call earlier in the same test turned out to have the identical problem: the app only reacts once the browser's own, separately-dispatched `resize` event on `window.visualViewport` is handled, which could itself lag behind the now near-instant clock-driven steps under load.
  3. Replacing that with the same technique the other two tests already use — directly overriding `visualViewport.height` via `Object.defineProperty` and dispatching the `resize` event *from the test itself* — made it deterministic, since the event is then handled synchronously within that call rather than through an async, real, contention-prone browser pipeline.

  Lesson for future tests in this file: prefer `page.clock` over real wall-clock waits, and prefer directly mocking + dispatching `visualViewport` events over `page.setViewportSize`, when the assertion needs to be tight — mixing a fast/deterministic technique with a slower/real one in the same test reintroduces the race the fast technique was meant to remove.
- **Keyboard-safe layout (pan)**: height alone wasn't enough on a real device — the height-only fix passed this exact test suite yet a real iOS Safari keyboard still panned the whole page, because `position: fixed` elements are pinned to the (unshrunk) *layout* viewport, and iOS shifts the *visual* viewport down over it (exposed as `visualViewport.offsetTop`) rather than resizing anything. The component now also tracks `offsetTop`/`offsetLeft` and applies them as a `translate` on the fixed container so it stays aligned with whatever's actually visible. The test mocks `visualViewport.height` *and* `offsetTop` together, fires `resize` and `scroll` (the event iOS fires while panning), and asserts the guess input's bounding rect stays within the mocked visible window. `index.html`'s viewport meta also gained `interactive-widget=resizes-content`, which makes supporting browsers (Chrome 108+) resize the layout viewport for real instead of panning at all — Safari doesn't support it yet, which is why the JS fallback above is still required.
- **Page scroll lock**: on top of the above, `WhoAmIGame` takes itself out of document flow (`position: fixed`) and sets `document.body`/`documentElement` to `overflow: hidden` for as long as it's mounted, restoring the previous value on unmount, as a last line of defense against the page itself scrolling. Both the effect that applies the lock and the one that restores it trail the route change slightly (a render/effect-timing gap, not related to the keyboard), so the test `expect.poll`s `getComputedStyle(document.body).overflow` rather than asserting immediately in either direction.
